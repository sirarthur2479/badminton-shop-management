...
- [ ] Shop Products inline edit form (from TASK-005) shows `sale_price` and `sale_ends_at` fields
- [ ] Setting `sale_price` on a product causes it to show a sale badge on `/shop` immediately after cache expires
- [ ] Sale badge condition: `sale_price` is set AND (`sale_ends_at` is null OR `sale_ends_at` > now)
- [ ] Product card shows: `<s>$X.XX</s>` original price + red `$Y.YY` sale price
- [ ] Expired sales (`sale_ends_at` in the past) are silently treated as no-sale — no badge, original price shown
- [ ] Clearing `sale_price` removes the sale badge
- [ ] A "Sales" filtered view in the Shop Products tab shows only products currently on sale (sale_price set, not expired)
